...
export default function DrawHistory() {
  ...
  const [displayMode, setDisplayMode] = useState("sorted"); // "separate" or "sorted"
  ...
  useEffect(() => {
    ...
    const games = [
      ["data_lotto6aus49", setLotto49Data],
      ["data_eurojackpot", setEurojackpotData],
      ["data_keno", setKenoData],
      ["data_super6", setSuper6Data],
      ["data_spiel77", setSpiel77Data],
      ["data_glücksrad", setGlücksradData],
    ];
    ...
    const fetchData = async () => {
      ...
      try {
        await Promise.all(
          games.map(([endpoint, setter]) =>
            fetchGame(`${API}/${endpoint}`, setter)
          )
        );
      } catch (err) {
